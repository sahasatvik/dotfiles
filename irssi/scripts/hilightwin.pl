...
sub sig_printtext {
    ...
    if(
        ...
        if ($dest->{level} & MSGLEVEL_PUBLIC) {
            $text = $text." (\e[0;34m".$dest->{target}."\e[0;37m)" if (!$shownetwork);
            $text = $text." (\e[0;34m".$dest->{server}->{tag}."/".$dest->{target}."\e[0;37m)" if ($shownetwork);
        } elsif ($shownetwork) {
            $text = $text." (\e[0;34m".$dest->{server}->{tag}."\e[0;37m)";
        }
        $text =~ s/%/%%/g;
